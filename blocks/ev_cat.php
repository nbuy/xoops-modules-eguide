...
# $Id: ev_cat.php,v 1.1 2008/02/03 15:28:51 nobu Exp $
...
function b_event_select_base($dirname, $prefix, $options) {
    ...
    $result = $xoopsDB->query("SELECT c.catid, c.catname AS name, c.catimg AS image,if (p.weight, p.weight, c.weight) ord1, if(p.weight IS NULL, -1, c.weight) ord2,c.catpri FROM $cat c LEFT JOIN $cat p ON c.catpri=p.catid ORDER BY ord1,ord2,catid");
    $list = array();
    $catid = isset($_GET['cat'])?intval($_GET['cat']):0;
    while ($data=$xoopsDB->fetchArray($result)) {
	$id = $data['catid'];
	$name = htmlspecialchars($data['name']);
	if (!empty($data['catpri'])) $name = '-- '.$name;
