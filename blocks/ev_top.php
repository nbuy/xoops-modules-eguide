...
if (!function_exists('b_event_top_show_base')) {
    ...
    function b_event_top_show_base($dirname, $prefix, $options)
    {
        ...
        if ($cat) {
            ...
            if ($ids) {
                $cond = 'catid IN (' . implode(',', $ids) . ')';
            }
            if ($labs) {
                if ($cond) {
                    $cond .= ' OR ';
                }
                $cond .= 'catname IN (' . implode(',', $labs) . ')';
            }
            if ($cond) {
                ...
                while (list($id, $name, $img) = $xoopsDB->fetchRow($res)) {
                    $ids[$id] = ['name' => htmlspecialchars($name), 'img' => $img];
                }
                $cond = $ids ? ' AND topicid IN (' . implode(',', array_keys($ids)) . ')' : '';
            }
        }
        ...
        switch ($list_type) {
            case 1:            // list of one entry for same event
                $sql = 'SELECT eid, title, summary, style, MIN(IF(exdate,exdate,edate)) edate, cdate, uid FROM '
                       . $xoopsDB->prefix($prefix)
                       . ' LEFT JOIN '
                       . $xoopsDB->prefix($prefix . '_extent')
                       . " ON eid=eidref AND exdate>$now WHERE (edate>$now OR exdate) $cond AND status=0 GROUP BY eid ORDER BY cdate DESC";
                break;
            case 2:            // list of one entry for expired event
                $sql      = 'SELECT eid, title, summary, style, MAX(IF(exdate,exdate,edate)) edate, cdate, uid FROM '
                            . $xoopsDB->prefix($prefix)
                            . ' LEFT JOIN '
                            . $xoopsDB->prefix($prefix . '_extent')
                            . " ON eid=eidref AND exdate<$now WHERE (edate<$now OR exdate) $cond AND status=0 GROUP BY eid ORDER BY cdate DESC";
  ...
  AND status=0 ORDER BY edate";
        ...
        while ($myrow = $xoopsDB->fetchArray($result)) {
            ...
            if (XOOPS_USE_MULTIBYTES) {
                if (function_exists('mb_strcut') && strlen($title) >= $nlen) {
                    $title = $myts->htmlSpecialChars(mb_strcut($myrow['title'], 0, $nlen - 1, _CHARSET)) . '...';
                }
            } else {
                if (strlen($title) >= $nlen) {
                    $title = $myts->htmlSpecialChars(substr($myrow['title'], 0, $nlen - 1)) . '...';
                }
            }
    ...
    function b_event_top_edit($options)
    {
        ...
        foreach (explode('|', _BLOCK_EV_TYPES) as $k => $v) {
            $typesel .= "<option value='$k'" . ($k == $list_type ? ' selected' : '') . ">$v</option>\n";
        }
        $typesel .= '</select>';

        return _BLOCK_EV_STYLE
               . '&nbsp;'
               . "<input type='radio' name='options[]' value='1'$sel0>"
               . _YES
               . " &nbsp; \n"
               . "<input type='radio' name='options[]' value='0'$sel1>"
               . _NO
               . "<br>\n"
               . _BLOCK_EV_ITEMS
               . "&nbsp;<input name='options[]' value='$nitem'><br>\n"
               . _BLOCK_EV_TRIM
               . "&nbsp;<input name='options[]' value='$nlen'><br>\n"
               . _BLOCK_EV_LISTTYPE
               . "&nbsp;$typesel<br>\n"
               . _BLOCK_EV_CATEGORY
               . "&nbsp;<input name='options[]' value='$cat'>\n"
