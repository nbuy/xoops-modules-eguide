            ...
            switch ($ty) {
                ...
                case 'const':
                    if (!$hidden) {    // pseudo POST variable
                        $_POST[$iname] = $result[$fname] = apply_user_vars(implode(',', $a));
                    }
                // no break
                case 'checkbox':
                case 'textarea':
...
function check_prev_order($data, $vals, &$errs, $force = false)
{
    ...
    if ($force || ACCEPT_EMAIL == $mo || (ACCEPT_BOTH == $mo && !is_object($xoopsUser))) {
        $email = param('email', '');
        ...
        if (!is_object($xoopsUser)) {
            redirect_header(Request::getString('HTTP_REFERER', '', 'SERVER'), 2);
        }
