...
switch ( $op ) {
	...
	case 'impsave':
		...
		while ( $data = $xoopsDB->fetchArray( $res ) ) {
			if ( ! $xoopsDB->query( 'INSERT INTO ' . CATBL . ' VALUES (' . join( ',', array_map( 'dbquote', $data ) ) . ')' ) ) {
				die( 'DB ERROR' );
			}
		}
		redirect_header( "?op=category", 1, _AM_DBUPDATED );
...
switch ( $op ) {
	case 'events':
		...
		if ( $result ) {
			...
			echo "<p><a class='ui-btn' href='?op=notifies'>" . _MD_INFO_REQUEST . "</a> <span class='badge'>" . sprintf( _MD_INFO_COUNT, $n ) . "</span></p>\n";
		}
		CloseTable();
		break;
...
function edit_category( $catid ) {
	...
	$formCatID   = new XoopsFormHidden( 'catid', $catid );
	$formCatSave = new XoopsFormHidden( 'op', 'catsave' );
	$formCatName = new XoopsFormText( _AM_CAT_NAME, 'catname', 40, 40, $data['catname'] );
	$formCatImg  = new XoopsFormText( _AM_CAT_IMG, 'catimg', 60, 191, $data['catimg'] );
	$formCatDesc = new XoopsFormDhtmlTextArea( _AM_CAT_DESC, 'catdesc', $data['catdesc'] );


	$form->addElement( $formCatID );
	$form->addElement( $formCatSave );
	$form->addElement( $formCatName, true );
	$form->addElement( $formCatImg );
	$form->addElement( $formCatDesc );
	...
	$formWeight = new XoopsFormText( _AM_WEIGHT, 'weight', 4, 4, $data['weight'] );
	$formBtn    = new XoopsFormButton( '', 'catsave', _SUBMIT, 'submit' );
	$form->addElement( $formWeight );
	$form->addElement( $formBtn );
...
function import_category() {
	...
	$formImpSave   = new XoopsFormHidden( 'op', 'impsave' );
	$formBtnSubmit = new XoopsFormButton( '', 'impsave', _SUBMIT, 'submit' );
	$form->addElement( $formImpSave );
	$form->addElement( $selmod );
	$selmod->setDescription( _AM_CAT_IMPORTDESC );
	$form->addElement( $formBtnSubmit );
	$form->display();
}
