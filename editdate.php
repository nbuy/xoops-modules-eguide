...
if ($xoopsDB->getRowsNum($result) > 0) {
    redirect_header(empty(Request::getString('HTTP_REFERER', '', 'SERVER')) ? EGUIDE_URL . '/admin.php' : Request::getString('HTTP_REFERER', '', 'SERVER'), 1, _NOPERM);
}
...
$GLOBALS['xoopsOption']['template_main'] = EGPREFIX . '_editdate.tpl';
assign_module_css();
$edate = $event['edate'];
...
$errors  = [];
if (isset($_POST['adds'])) {
    $dels    = empty($_POST['dels']) ? [] : $_POST['dels'];
    $mods    = $_POST['mods'];
    $exps    = $_POST['exps'];
    $adds    = preg_split('/[\n\r]+/', trim($_POST['adds']));
    $chg     = 0;
    ...
    foreach ($extents as $data) {
        ...
        if (isset($dels[$id])) {
            if ($data['exdate'] > $now && $data['reserved']) {
                ...
                $updated .= sprintf("(id:%s) %s [%s]\n", "$eid-$exid", _DELETE, formatTimestamp($data['exdate'], _MD_POSTED_FMT));
                ++$chg;
            }
            ...
            $v      = trim($mods[$id]);
            if ($pre == $v && $exps[$id] == $data['expersons']) {
                continue;
            }
            $n = '' == $exps[$id] ? 'null' : (int)$exps[$id];
            list($date, $time) = explode(' ', trim($mods[$id]));
