...
require_once __DIR__ . '/notify.inc.php';
...
$targs = ['title', 'summary', 'body', 'optfield', 'before'];
define('_EG_OPTDEFS', 'redirect=' . _MD_RESERV_REDIRECT . ",text\n");

$myts                                    = MyTextSanitizer::getInstance();
$GLOBALS['xoopsOption']['template_main'] = EGPREFIX . '_admin.tpl';

if ('new' === $op) {
    // default value in post form
    $data = [
        'lang_event_edit' => _MD_NEWTITLE,
    ...
    if ('preview' === $op || 'save' === $op || 'date' === $op) {
        $edate = getDateField('edate');
        if (isset($_POST['expire'])) {
            $expire = empty($_POST['expire']) ? time_to_sec($_POST['expire_text']) : (int)$_POST['expire'];
        } else {
...
if ('save' === $op || 'date' === $op) {
    ...
    if ($pdata['status'] != $data['status']) {
        user_notify($eid);
    }
    $result = $xoopsDB->query('SELECT * FROM ' . OPTBL . " WHERE eid=$eid");

    $ofields = [
        'reservation' => _MD_RESERV_DESC,
    ...
    if ($updated && STAT_NORMAL == $data['status']) {
...
if (!empty($ev_month)) {
    $xoopsTpl->assign('monthname', $ev_month);
}
$y = formatTimestamp(time(), 'Y');
$xoopsTpl->assign('calrange', [$y - 10, $y + 10]);

$check = ['title' => _MD_TITLE, 'summary' => _MD_INTROTEXT];
...
if ($tstr) {
    foreach (explode(',', $tstr) as $value) {
        ...
        $timetable[] = ['label' => $label, 'value' => $value];
    }
}
$xoopsTpl->assign('timetable', $timetable);

if ($eid && 'delete' === $op) {
    $GLOBALS['xoopsOption']['template_main'] = EGPREFIX . '_event.tpl';
    edit_eventdata($data);
...
{
    $unit = explode(',', _MD_TIME_UNIT);
    if ((abs($sec) >= 86400) && 0 == ($sec % 86400)) { // days

        return ($sec / 86400) . $unit[0];
    } elseif ((abs($sec) >= 3600) && (0 == $sec % 3600)) { // hours

        return ($sec / 3600) . $unit[1];
    }

    return (int)($sec / 60) . $unit[2];
}
