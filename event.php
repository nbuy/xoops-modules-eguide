...
elseif (count($extents) && 0 == $exid) {
    if (1 == count($extents)) {    // only one extent, chose that.
...
set_eguide_breadcrumbs($data['topicid'], [$title => '']);
...
if ($data['closedate'] < $now) {
    ...
    } elseif (!is_object($xoopsUser) && ACCEPT_MEMBER == $xoopsModuleConfig['member_only']) {
...
if ('print' === $op) {
    $GLOBALS['xoopsOption']['template_main'] = EGPREFIX . '_event_print.tpl';
