...
if (isset($_GET['line'])) {
    $max_rows = (int)$_GET['line'];
    $max_sect = max(1, (int)(40 / $max_rows));
} elseif (1 == $xoopsDB->getRowsNum($result)) { // only one page
    $max_sect = 1;
...
while ($data = $xoopsDB->fetchArray($result)) {
    ...
    if (empty($sheets[$tab])) {
        ...
        $sheets[$tab] = ['title' => $data['title'], 'edate' => $day, 'rows' => []];
    }
