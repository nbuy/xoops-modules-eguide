...
if (isset($catlist[$catid])) {
    $vals = [$catid];
    if (0 == $catlist[$catid]['catpri']) { // also children categories
    ...
    $opt = ' AND topicid IN (' . implode(',', $vals) . ')';
} else {
...
$result = $xoopsDB->query('SELECT ' . $fields . ' FROM ' . EGTBL . ' e LEFT JOIN ' . OPTBL . ' o ON e.eid=o.eid LEFT JOIN ' . EXTBL . " x ON e.eid=eidref $ext
...
$GLOBALS['xoopsOption']['template_main'] = EGPREFIX . '_index.tpl';
