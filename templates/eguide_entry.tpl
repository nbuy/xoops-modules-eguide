...
<tr class="<{cycle values="even,odd"}>"><td class="head"><{$smarty.const._MD_UNAME}></td><td><{$form.uname}></td></tr>
<{if !$form.member_only}>
<tr class="<{cycle values="even,odd"}>"><td class="head"><{$smarty.const._MD_EMAIL}></td><td><input type="text" size="40" name="email" value="<{$form.email}>" class="form-control" />
</td></tr>
<{/if}>
...
<{/if}>
<p style="text-align: center;"><input type="submit" value="<{$smarty.const._MD_ORDER_SEND}>" class="btn btn-primary" /></p>
</form>
</div>
<{if $form.note}>
