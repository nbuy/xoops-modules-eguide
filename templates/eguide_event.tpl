...
<form action="reserv.php?op=<{$form.op}>#form" name="order" method="post" onsubmit="return xoopsFormValidate_order();" class="form-inline">
...
<{if !$form.member_only}>
<tr class="evms <{cycle values="even,odd"}>"><td class="head"><{$smarty.const._MD_EMAIL}><{$smarty.const._MD_REQUIRE_MARK}></td><td><input type='text' size="40" name="email" id="email" value="<{$form.email}>" class="form-control" />
<{if $form.user_notify}>
<br/><label class="checkbox-inline"><input type="checkbox" <{if $smarty.post.notify}>checked="checked" <{/if}>name="notify" /><{$smarty.const._MD_NOTIFY_REQUEST}></label>
<{/if}>
</td></tr>
<{if $form.options.email_repeat_check}>
<tr class="evms <{cycle values="even,odd"}>"><td class="head"><{$smarty.const._MD_EMAIL_CONF}><{$smarty.const._MD_REQUIRE_MARK}></td><td><input type="text" size="40" name="email_conf" id="email_conf" value="<{$form.email_conf}>" class="form-control" />
...
<{foreach from=$form.items item=fm name=item}>
  <{if preg_match("/^-/", $form.items[$smarty.foreach.item.iteration].label)}>
    <{if empty($form_continue)}>
      <tr class="<{$fm.attr}> <{cycle values="even,odd"}>"><td class="head"><{$fm.label}></td><td>
  ...
  <{else}>
    ...
    <{else}>
      <tr class="<{$fm.attr}> <{cycle values="even,odd"}>"><td class="head"><{$fm.label}></td><td><{$fm.value}><{if $fm.comment}><span class="note"><{$fm.comment}></span><{/if}></td></tr>
    <{/if}>
  <{/if}>
<{/foreach}>
</table>
...
<p style="text-align: center;">
    ...
    <input type="submit" value="<{if $form.op=="confirm"}><{$smarty.const._MD_ORDER_CONF}><{else}><{$smarty.const._MD_ORDER_SEND}><{/if}>" <{$form.submit_opts}> class="btn btn-primary" />
...
myobj = myform["<{$name}>"];
if ( checkItem(myobj) ) { msg = msg+"<{$msg}>\n"; if(obj==null)obj=myobj; }
<{/foreach}>
<{if $form.options.email_repeat_check}>
