...
<{/if}>
<input type="text" name="search" value="<{$smarty.get.search|escape}>" class="form-control" /> <input type="submit" value="<{$smarty.const._SEARCH}>" class="btn btn-default" />
</form>
</div>
<table width="100%">
  <tr>
...
<table class="outer table table-bordered table-striped" align="center" cellspacing="1" border="0">
  ...
  <tr class="<{cycle values="even,odd"}>">
    <td>
      ...
      <{if $xoops_isadmin && $user.uid>0}>| <a href="mylist.php?eid=<{$form.eid}><{if $event.exid}>&amp;exid=<{$event.exid}><{/if}>&amp;uid=<{$user.uid}>"><{$smarty.const._MD_DETAIL}></a> <{/if}>
    </td>
    <{foreach item=col from=$columns}>
