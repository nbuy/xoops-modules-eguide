...
<div class="footer">
    ...
    <div>
        <a href="<{$smarty.const.EGUIDE_URL}>/event.php?eid=<{$event.eid}><{if $event.exid}>&sub=<{$event.exid}><{/if}>"><{$smarty.const.EGUIDE_URL}>
            /event.php?eid=<{$event.eid}><{if $event.exid}>&amp;sub=<{$event.exid}><{/if}></a></div>
</div>
</body>
</html>
