...
if (isset($_POST['persons'])) {
    ...
    foreach ($_POST['persons'] as $k => $v) {
        ...
        if (preg_match('/^(\\d+)-(\\d+)$/', $k, $d)) {
            $eid  = (int)$d[1];
            $exid = (int)$d[2];
            $cond = ('null' === $v) ? 'expersons IS NOT NULL' : "(expersons IS NULL OR expersons<>$v)";
    ...
    if ($updated) {
        include __DIR__ . '/notify.inc.php';
        $res     = $xoopsDB->query('SELECT eid,title,uid FROM ' . EGTBL . ' WHERE eid IN (' . implode(',', array_keys($ids)) . ') AND status=' . STAT_NORMAL);
        $urls    = '';
    ...
    $url = isset(Request::getString('HTTP_REFERER', '', 'SERVER')) ? Request::getString('HTTP_REFERER', '', 'SERVER') : EGUIDE_URL . '/collect.php';
...
if ($xoopsUser->isAdmin($xoopsModule->getVar('mid'))) {
    if (isset($_GET['uid'])) {
        $cond .= ' AND uid=' . (int)$_GET['uid'];
    }
} else {
    $cond .= ' AND uid=' . $xoopsUser->getVar('uid');
}
if (isset($_GET['eid'])) {
    $cond .= ' AND e.eid=' . (int)$_GET['eid'];
}
...
$GLOBALS['xoopsOption']['template_main'] = EGPREFIX . '_collect.tpl';
assign_module_css();

$num = $xoopsDB->getRowsNum($result);

$dateline = $timeline = $cells = $event = [];
